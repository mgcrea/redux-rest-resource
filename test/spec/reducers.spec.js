...
describe('defaultReducers', () => {
  ...
  it('should handle CREATE action', () => {
    ...
    expect(pendingState).toEqual({isCreating: true});

    status = 'resolved';
    const body = {id: 1, firstName: 'Olivier'};
    const receivedAt = Date.now();
    const resolvedAction = {type, status, context, body, receivedAt};
    expect(reducers[actionId](pendingState, resolvedAction)).toEqual({isCreating: false, items: [body]});

    status = 'rejected';
    const rejectedAction = {type, status, context, err: {}, receivedAt};
    expect(reducers[actionId](pendingState, rejectedAction)).toEqual({isCreating: false});
  });
  it('should handle FETCH action (classic fetch)', () => {
    ...
    expect(pendingState).toEqual({isFetching: true, didInvalidate: false});

    status = 'resolved';
    const code = 200;
    const body = [{id: 1, firstName: 'Olivier'}];
    const receivedAt = Date.now();
    expect(reducers[actionId](pendingState, {type, status, body, receivedAt, code})).toEqual({
      isFetching: false,
      didInvalidate: false,
      items: body,
    ...
    status = 'rejected';
    expect(reducers[actionId](pendingState, {type, status, err: {}, receivedAt})).toEqual({
      didInvalidate: false,
      isFetching: false
    });
  });
  describe('should handle FETCH action with Content-Range header', () => {
    it('partial-content fetch => first range [0 to 2]', () => {
      ...
      const receivedAt = Date.now();
      expect(reducers[actionId](pendingState, {type, status, body, receivedAt, code, contentRange})).toEqual({
        isFetching: false,
        didInvalidate: false,
        items: body,
        lastUpdated: receivedAt
    ...
    it('partial-content fetch => first range [0 to 2] with 3 old items', () => {
      ...
      expect(pendingState).toEqual({isFetching: true, didInvalidate: false, items: oldItems});
  ...
  it('should handle GET action', () => {
    ...
    expect(pendingState).toEqual({isFetchingItem: true, didInvalidateItem: false});

    status = 'resolved';
    const body = [{id: 1, firstName: 'Olivier'}];
    const receivedAt = Date.now();
    expect(reducers[actionId](pendingState, {type, status, context, body, receivedAt})).toEqual({
    ...
    status = 'rejected';
    expect(reducers[actionId](pendingState, {type, status, context, err: {}, receivedAt})).toEqual({
      isFetchingItem: false,
      didInvalidateItem: false
    });
  });
  it('should handle UPDATE action', () => {
    ...
    expect(pendingState).toEqual({...customInitialState, isUpdating: true});
  ...
  it('should handle DELETE action', () => {
    ...
    status = 'pending';
    const pendingState = reducers[actionId](customInitialState, {type, status, context});
    expect(pendingState).toEqual({...customInitialState, isDeleting: true});
...
describe('custom reducers', () => {
  ...
  it('should handle RUN action', () => {
    ...
    const pendingAction = {type, status, context};
    const pendingState = reducers[actionId](undefined, pendingAction);
    expect(pendingState).toEqual({isRunning: true});

    status = 'resolved';
    const body = {ok: 1};
    const receivedAt = Date.now();
    const resolvedAction = {type, status, context, body, receivedAt};
    expect(reducers[actionId](pendingState, resolvedAction)).toEqual({isRunning: false});

    status = 'rejected';
    const rejectedAction = {type, status, context, err: {}, receivedAt};
    expect(reducers[actionId](pendingState, rejectedAction)).toEqual({isRunning: false});
  });
  it('should handle FETCH action', () => {
    ...
    status = 'pending';
    const pendingState = reducers[actionId](undefined, {type, status});
    expect(pendingState).toEqual({isMerging: true});

    status = 'resolved';
    const body = [{id: 1, firstName: 'Olivier'}];
    const receivedAt = Date.now();
    expect(reducers[actionId](pendingState, {type, status, body, receivedAt})).toEqual({isMerging: false});

    status = 'rejected';
    expect(reducers[actionId](pendingState, {type, status, err: {}, receivedAt})).toEqual({isMerging: false});
  });
});
...
describe('custom pure reducers', () => {
  ...
  it('should handle CLEAR action', () => {
    ...
    const receivedAt = Date.now();
    const resolvedAction = {type, status, context, body, receivedAt};
    expect(reducers[actionId](undefined, resolvedAction)).toEqual({item: null});
  });
});
  ...
  describe('`assignResponse` option', () => {
    it('should handle GET action', () => {
      ...
      const reducers = createReducers(
        {...defaultActions, [actionId]: {...defaultActions[actionId], assignResponse}},
        {resourceName}
      );
      const type = types[getActionTypeKey(actionId, {resourceName})];
      ...
      status = 'pending';
      const pendingState = reducers[actionId](customInitialState, {type, status, context});
      expect(pendingState).toEqual({...customInitialState, isFetchingItem: true, didInvalidateItem: false});

      status = 'resolved';
      const body = {id: 1, firstName: 'Olivia', lastName: 'Louvignes'};
      const receivedAt = Date.now();
      const expectedItems = [body];
      expect(reducers[actionId](pendingState, {type, status, context, options, body, receivedAt})).toEqual({
        isFetchingItem: false,
        didInvalidateItem: false,
        items: expectedItems,
        item: body,
      ...
      status = 'rejected';
      expect(reducers[actionId](pendingState, {type, status, context, options, err: {}, receivedAt})).toEqual({
        ...customInitialState,
        didInvalidateItem: false,
        isFetchingItem: false
      });
...
describe('rootReducer', () => {
  it('should handle a default action', () => {
    ...
    expect(pendingState).toEqual({...initialState, isCreating: true});

    status = 'resolved';
    const body = {id: 1, firstName: 'Olivier'};
    const receivedAt = Date.now();
    const resolvedAction = {type, status, context, body, receivedAt};
    expect(rootReducer(pendingState, resolvedAction)).toEqual({...initialState, isCreating: false, items: [body]});

    status = 'rejected';
    const rejectedAction = {type, status, context, err: {}, receivedAt};
    expect(rootReducer(pendingState, rejectedAction)).toEqual({...initialState, isCreating: false});
  });
  it('should handle a custom action', () => {
    ...
    const pendingAction = {type, status, context};
    const pendingState = rootReducer(undefined, pendingAction);
    expect(pendingState).toEqual({...initialState, isRunning: true});

    status = 'resolved';
    const body = {id: 1, firstName: 'Olivier'};
    const receivedAt = Date.now();
    const resolvedAction = {type, status, context, body, receivedAt};
    expect(rootReducer(pendingState, resolvedAction)).toEqual({...initialState, isRunning: false});

    status = 'rejected';
    const rejectedAction = {type, status, context, err: {}, receivedAt};
    expect(rootReducer(pendingState, rejectedAction)).toEqual({...initialState, isRunning: false});
  });
  describe('should handle a pure action', () => {
    ...
    expect(rootReducer(initialState, resolvedAction)).toEqual({...initialState, items: [{foo: 'bar'}]});
  });
});
...
describe('helpers', () => {
  describe('combineReducers', () => {
    it('should properly combine two reducer functions as a single object', () => {
      const fooReducers = createRootReducer(createReducers(defaultActions, {resourceName: 'foo'}), {
        resourceName: 'foo'
      });
      const barReducers = createRootReducer(createReducers(defaultActions, {resourceName: 'bar'}), {
        resourceName: 'bar'
      });
